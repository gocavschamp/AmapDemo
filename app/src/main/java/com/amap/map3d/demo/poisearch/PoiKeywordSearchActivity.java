...
//		String type = "190100|190400";
		String type = "190000|190100|170000|190400";
		query = new PoiSearch.Query(keyWord, type, editCity.getText().toString());// 第一个参数表示搜索字符串，第二个参数表示poi搜索类型，第三个参数表示poi搜索区域（空字符串代表全国）
				...
				if (result.getQuery().equals(query)) {// 是否是同一条
					...
					if (poiItems != null && poiItems.size() > 0) {
						...
						for (int i = 0; i < poiItems.size(); i++) {
							...
							String typeDes = poiItems.get(i).getAdName();
							Log.e("tag---",title+"-"+snippet+"-"+cityName+"-"+typeDes);
						}
